<?php
	...
	class NewsMacros {
		 ...
		 * что соответствует также значению параметра, равному «+ -».
		 * Формат строки-значения параметра следующий: «[+-]?\d?[mhd]?\s?[+-]?\d?[mhd]?».
		 *
		...
		public function view($elementPath = '', $template = 'default') {
			...
			return $moduleClass::parseTemplate($template_block, [
				'id' => $element->getId(),
			], $element->getId());
		}
