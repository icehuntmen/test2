		...
		public function tag() {
			...
			if ($sReturnMode == 'xml') {
				...
				foreach($aRet as $aRow) {
					$sAnswer .= '<row uri="' . $aRow['uri'] . '" count="' . $aRow['count'] . '" rel="' . number_format($aRow['rel'] * 100, 2, '.', '') . '" />';
				}
				$sAnswer .= "	</data>\n 	</report>\n</statistics>";
...
END;
				...
				if ($iRest > 0) {
					$sAnswer .= "<row cnt=\"{$iRest}\" name=\"" . getLabel('label-other') . '" uri="" rel="' . round($iRest / ($iTotalAbs / 100), 1) . '" />';
				}
				$sAnswer .= "</data>\n";
				$sAnswer .= "</report>\n</statistics>";
...
END;
				...
				$iRest = ($iTotalAbs - $iHoveredAbs);
				if ($iRest > 0) {
					$sAnswer .= "<row count=\"{$iRest}\" name=\"" . getLabel('label-other') . '" uri="" rel="' . round($iRest / ($iTotalAbs / 100), 1) . '" />';
				}
				$sAnswer .= "</data>\n";
				$sAnswer .= "</report>\n</statistics>";
...
END;
				...
				while(($iOldTimeStamp < $this->module->to_time + 86400) && (date($sCmpFmt, $iOldTimeStamp) != $sThisDate)) {
					$attr_page_uri   = htmlspecialchars( '' );
					$sAnswer .= '<row ' .
						'timestamp="' . $iOldTimeStamp . '" ' .
						'count="0" ' .
...
END;
				...
				$labelChangesInAverageGapBetweenVisitorsReturns = getLabel('label-changes-in-average-gap-between-visitors-returns');
...
END;
				...
				$labelVisitsRelative = getLabel('label-visits-relative');
				$labelVisitsCount = getLabel('label-visits-count');
				$labelDistributionOfVisitsByTimeSpentOnSite = getLabel('label-distribution-of-visits-by-time-spent-on-site');
...
END;
				...
				$labelDynamicsOfAverageLengthOfTimeVisitorsToSite = getLabel('label-dynamics-of-average-length-of-time-visitors-to-site');
		...
		public function auditoryLocation() {
			...
			if ($sReturnMode == 'xml') {
				...
				<report name="auditoryLocation" title="' . $labelDistributionOfAudienceByCity . '"
						host="'. $thisHost . '" lang="' . $thisLang . '" timerange_start="' . $this->module->from_time . '" timerange_finish="' . $this->module->to_time . '">
				<table>
