<?php
	...
	class OneCExchange {
		...
		protected function saveIncomingFile() {
			...
			if (!mb_strlen($extension) || !umiFile::isAllowedFileType($extension)) {
				return "failure\nUnknown file type.";
			}
			...
			while (($data = fgets($handle, 1000)) !== false) {
				...
				switch ($type) {
					...
					case 'property': {
						...
						if (!filter_var($typeId, FILTER_VALIDATE_INT) || !$umiObjectTypesCollection->getType($typeId) instanceof iUmiObjectType) {
							$this->reportError(getLabel('error-no-type', false, $typeId));
						}

						if (!filter_var($fieldId, FILTER_VALIDATE_INT) || !$umiFieldsCollection->getField($fieldId) instanceof iUmiField) {
