<?php
	...
	class CommentsMacros {
		...
		public function post($parent_element_id = false) {
			...
			if (!mb_strlen(trim($title)) && ($parentElement instanceof iUmiHierarchyElement)) {
			...
			while (mb_strpos($referrer_url, '&&') !== false || mb_strpos($referrer_url, '??') !== false || mb_strpos($referrer_url, '?&') !== false) {
				$referrer_url = str_replace('&&', '&', $referrer_url);
				$referrer_url = str_replace('??', '?', $referrer_url);
				$referrer_url = str_replace('?&', '?', $referrer_url);
			}

			if (mb_strlen($referrer_url) && (mb_substr($referrer_url, -1) === '?' || mb_substr($referrer_url, -1) === '&')) {
