<?php
	...
	class EmarketYandexMarketClient {
		...
		public function yandexMarketCreateToken() {
			...
			if (mb_strlen($clientId)==0 ||mb_strlen($password)==0 ) {
		...
		public function getYandexMarketSettings($domainId = false) {
			...
			$settings = [];
			$settings['clientId'] = $umiRegistry->get("//modules/emarket/yandex_market/{$domainId}/clientId");
			...
			if (mb_strlen($settings['clientId']) == 0) {
				return false;
			}
