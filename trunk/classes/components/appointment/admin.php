<?php
	...
	class AppointmentAdmin {
		...
		public function addPage() {
			$modeKey = 'param2';
			$type = 'page';
			$mode = (string) getRequest($modeKey);
		...
		public function serviceWorkingTime() {
			...
			if ($isSaveMode) {
				...
				foreach ($schedules as $dayNumber => $formData) {
					...
					if (isset($formData['from']) && is_string($formData['from']) && preg_match($timeValidatePattern, $formData['from'])) {
			...
			$options = $this->prepareData($config, 'settings');
			$this->setActionType('modify');
		...
		public function config() {
			...
			$this->setDataType('settings');
			$options = $this->prepareData($params, 'settings');
			$this->setActionType('modify');
			$this->setData($options);
			$this->doData();
		}
		...
		protected function saveEmployeeSchedules(AppointmentEmployee $employee) {
			...
			foreach ($schedules as $dayNumber => $schedule) {
				if (!isset($schedule['from']) || !is_string($schedule['from']) || !preg_match($timeValidatePattern, $schedule['from'])) {
					continue;
				}
			...
			if ($mode == 'do') {
				...
				switch (getRequest('save-mode')) {
					...
					case getLabel('label-save-add'): {
						$this->module->redirect($this->module->pre_lang . '/admin/appointment/' . $editMethod . '/' . $entity->getId());
						break;
					}
				}
