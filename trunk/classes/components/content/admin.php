	...
	class ContentAdmin {
		...
		public function content_control() {
			...
			if ($mode == 'do') {
				...
				if ($umiNotificationInstalled) {
					$regedit->set('//settings/use-umiNotifications', $params['notifications']['boolean:use-umiNotifications']);
				}
			...
			if ($umiNotificationInstalled) {
				$params['notifications']['boolean:use-umiNotifications'] = $regedit->get('//settings/use-umiNotifications');
			}
		...
		protected function setCopyToLangPreventAltNamesDoublesResponse(
			iDomain $targetDomain, $languageSuffix, array $pagesWithExistingAltNames
		) {
			$this->setDataType('list');
			$this->setActionType('view');
			$data = [
		...
		public function copyElementToSite() {
			...
			foreach ($targetPages as $targetPageId) {
				...
				$altsHaveSameBase = mb_strpos($newPageAltName, $targetPageAlt) === 0;
