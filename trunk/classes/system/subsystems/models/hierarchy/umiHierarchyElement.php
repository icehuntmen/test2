...
SQL;
			...
			list($parentId, $typeId, $langId, $domainId, $tplId, $objectId, $ord, $altName, $active, $visible, $deleted,
				$updateTime, $default, $objectName, $objectTypeId, $objectIsLocked, $objectOwnerId, $objectGuid,
				$objectTypeGuid, $objectUpdateTime, $objectOrd) = $data;
			...
			if ($this->is_active && !$this->is_deleted) {
				...
				if ($parent instanceof iUmiHierarchyElement) {
					if ($parent->getHierarchyType()->getId() != $hierarchyCatalogCategoryType->getId()) {
						...
						for ($i = umiCount($parents) - 1; $i >= 0; $i--) {
							...
							if ($newParent instanceof iUmiHierarchyElement && $newParent->getHierarchyType()->getId() == $hierarchyCatalogCategoryType->getId()) {
				...
				if (is_array($matches)) {
					...
					foreach ($matches as $exportId) {
						...
						file_put_contents($dirName . 'shop' . $exportId, '<name>' . iconv('UTF-8', 'CP1251//IGNORE', $name) . '</name><company>' . iconv('UTF-8', 'CP1251', $companyName) . '</company><url>' . $domain->getUrl() . '</url>');
					}
				}
			}
		}
