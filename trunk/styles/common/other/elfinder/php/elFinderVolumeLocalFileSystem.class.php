...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if ($this->options['tmbPath']) {
			$this->options['tmbPath'] = mb_strpos($this->options['tmbPath'], DIRECTORY_SEPARATOR) === false
		...
		if ($this->attr($this->root, 'read') && !$this->tmbURL && $this->URL) {
			if (mb_strpos($this->tmbPath, $this->root) === 0) {
	...
	protected function _relpath($path) {
		return $path == $this->root ? '' : mb_substr($path, mb_strlen($this->root)+1);
	}
	...
	protected function _inpath($path, $parent) {
		return $path == $parent || mb_strpos($path, $parent.DIRECTORY_SEPARATOR) === 0;
	}
	...
	protected function _dimensions($path, $mime) {
		return mb_strpos($mime, 'image') === 0 && ($s = @getimagesize($path)) !== false
			? $s[0].'x'.$s[1]
			: false;
	}
	...
	protected function _readlink($path) {
		...
		if (mb_substr($target, 0, 1) != DIRECTORY_SEPARATOR) {
		...
		if ($this->_inpath($atarget, $this->aroot)) {
			return $this->_normpath($this->root.DIRECTORY_SEPARATOR.mb_substr($atarget, mb_strlen($this->aroot)+1));
		}
