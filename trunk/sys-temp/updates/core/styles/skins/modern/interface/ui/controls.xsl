...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="ui-new-table">
        <xsl:param name="controlParam">''</xsl:param>
        <xsl:param name="dragAllowed">'false'</xsl:param>
        <xsl:param name="configUrl">''</xsl:param>
	...
	<xsl:template name="ui-smc-table">
		...
		<script type="text/javascript">
			...
			// Объявление кастомного меню для тулбара табличного контрола
			var newFunc = null;
	...
	<xsl:template name="std-optioned-control">
		...
		<div>
			...
			<div class="layout-row-icon row">
				...
				<div class="col-md-5">
					<div class="layout-col-control">
						<input type="number" class="default edit number" umi:type="{$type}" autocomplete="off" />
					</div>
			...
			<div class="col-md-5">
				<div class="layout-col-control">
					<xsl:choose>
						...
						<xsl:otherwise>
							<input type="number" class="default" umi:type="float" name="{$input-name}[{$position}][float]">
								<xsl:attribute name="value">
									<xsl:choose>
										<xsl:when test="@float">
