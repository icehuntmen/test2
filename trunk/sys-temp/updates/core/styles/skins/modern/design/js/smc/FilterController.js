...
function filterController(_oControll, _iTypeId, _bSuspendInit, _oContainerEl, _Params) {
	...
	var initDomainSelect = function(select) {
		requestGet('/udata://core/getDomainsList/', function(response) {
			...
			for (var i = 0; i < domainList.length; i++) {
				appendOption(select, domainList[i].getAttribute('id'), domainList[i].getAttribute('host'));
			}
	...
	var buildFilterForm = function(_TypeDOM) {
		...
		quickSearch = buildInput({'type': 'text', 'class': 'default', 'name': 'quicksearch', 'id': fcId + '_id_quicksearch'});
