...
var TableItem = function(_oControl, _oParent, _oData, _oSiblingItem, _sInsertMode) {
	 ...
	 * @param {Number|null} entityId идентификатор связанной сущности, которая может влиять на результат экспорта
	 */
	this.exportCallback = function(entityId) {
		 ...
		 */
		var requestDownload = function(entityId, encoding, popupName) {
			closeDialog(popupName);
			var request = _self.getExportLink(entityId, encoding);
		 ...
		 */
		var requestExport = function(entityId, encoding, popupName, callback) {
			var request = _self.getExportLink(entityId, encoding);
			...
			$.ajax({
				...
				if (typeof response.is_complete === 'boolean') {
					...
					return requestExport(entityId, encoding, popupName, callback);
				}
		...
		$.get('/styles/skins/modern/design/js/smc/html/quickExportDialog.html', function(html) {
			...
			openDialog('', getLabel('js-csv-export'), {
				...
				confirmCallback: function(popupName) {
					...
					selectEncoding(entityId, function(encoding) {
						$('#quick_csv_encoding', $popup).hide();
						$('div.eip_buttons', $popup).hide();
						$('div.exchange_container', $popup).show();
						requestExport(entityId, encoding, popupName, function() {
							requestDownload(entityId, encoding, popupName);
						});
					});
				},
	 ...
	 */
	var selectEncoding = function(entityId, callback) {
	...
	/**
	 * Метод рисующий кнопки импорта экспорта в csv
