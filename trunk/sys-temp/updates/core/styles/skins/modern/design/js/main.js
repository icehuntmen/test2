$(function() {
	...
	if (typeof uAdmin.wysiwyg != 'undefined') {
		uAdmin.wysiwyg.init({
			settings: {
				content_css: '/styles/skins/modern/design/css/tinyMCE.css',
				height: '300'
			}
		});
	}
	...
	$('.round-toggle').click(function() {
		$(this).toggleClass('switch');
		var $container = $(this).parents('.panel-settings');
		$container.toggleClass('has-border');
		var groupTipButton = $(this).siblings('.group-tip-show');
		var isCollapsed = $(this).hasClass('switch');
...
function initOptionedFields() {
	...
	/**
	 * Добавляем DOM-элементы для заполнения составной опции
	 * @param {jQuery|Element} container элемент-контейнер для поля
...
/** инициализация слайдера для дашбоарда модулей */
