	...
	trait baseModuleAdmin {
		...
		public function prepareDataPage($inputData) {
			...
			if ($this->actionType == 'create') {
				...
				if (!$this->compareObjectTypeByHierarchy($module, $method, $type_id)) {
		...
		public static function getExpectParam($param_name, $param_type, $param = null) {
			...
			switch ($param_type) {
				...
				case 'select-multi': {
					return (is_array($value) && umiCount($value) > 0) ? implode(umiObjectPropertyRelation::DELIMITER_ID, $value) : '';
					break;
				}
		...
		public function saveEditedElementData($inputData) {
			...
			if (getRequest('alt-name') !== null) {
				$alt_name = mb_strlen(getRequest('alt-name')) ? getRequest('alt-name') : getRequest('name');
			...
			if (($is_active = getRequest('active')) !== null) {
				...
				if ($permissions->isAllowedMethod($user_id, $module_name, 'publish')) {
			...
			if (is_array($data)) {
				foreach ($data as $id => $info) {
					...
					if ($id == 'new') {
						...
						if ($host && $lang_id) {
							$edition = Service::Registry()->get('//modules/autoupdate/system_edition');
							...
							$collection->addDomain($host, $lang_id, false, $usingSsl);
						}
					} else {
						if (!$host) {
		...
		public function saveEditedLangs($data, $dels) {
			...
			if (is_array($data)) {
				foreach ($data as $id => $info) {
					...
					if (!mb_strlen($title) || !mb_strlen($prefix)) {
						continue;
					}
		...
		/**
		 * Добавить новое поле в тип данных
		 * @param array $inputData
		 * @return bool|int
		 * @throws publicAdminException
		 * @throws coreException
		...
		public function deleteObject($params) {
			\UmiCms\System\Protection\Security::getInstance()
				->checkCsrf();
		...
		final public function dataset_config() {
			...
			$childMap = ['methods' => 'method', 'types' => 'type', 'stoplist' => 'exclude', 'default' => 'column', 'fields' => 'field'];

			$datasetConfig = $this->module->getDatasetConfiguration($param);
							...
							if (is_array($record)) {
								foreach ($record as $propertyName => $propertyValue) {
									...
									if ($propertyName == 'id' && !is_numeric($propertyValue)) {
										$propertyValue = $objectTypes->getTypeIdByHierarchyTypeName(get_class($this->module), $propertyValue);
									}
		...
		public function saveFormDataToEntityWithDefinedType(
			$entityId = null, $entityType = null, array $editFormData = null)
		{
		...
		public function deleteEntitiesWithDefinedTypes(array $entitiesData = null) {
			...
			$this->setDataSetDeleteResult(
				$this->deleteCollectionsEntities(
					$entitiesData, $this->getModuleCollections()
				)
			);
		}
		...
		public function saveFieldOfEntityWithDefinedType(
			$entityId = null, $entityType = null, $fieldName = null, $fieldValue = null
		) {
			$entity = $this->getEntityByIdAndType($entityId, $entityType);
			...
			try {
				$fieldValue = $this->callbackBeforeSavingEntityField(
					$entity, $collection, $fieldName, $fieldValue
				);
		...
		public function moveEntitiesWithDefinedTypes(
			array $targetEntityData = null, array $draggedEntityDataList = null, $dragMode = null
		) {
		...
		public function initOptions() {
			...
			$this->forEachOption(function ($group, $option, $settingGroup, $settingName, $initial, $extra) use (&$result) {
		...
		public function getEntitiesForTable($serviceName, array $fieldNames) {
			...
			try {
				...
				$orders = (isset($_REQUEST['order_filter']) && is_array($_REQUEST['order_filter'])) ? $_REQUEST['order_filter'] : $defaultOrder;
		 ...
		 * @param iUmiConstantMap $entityFieldsConstantMap карта констант, в которой хранятся константы полей
		 * @return array
		 */
		...
		public function callbackBeforeSavingEntityField(
			iUmiCollectionItem $entity, iUmiCollection $collection, $fieldName, $fieldValue
		) {
		...
		public function callbackAfterCreateEntityFromFormData(
			iUmiCollectionItem $entity, iUmiCollection $collection) {
			return $entity;
		}
		 ...
		 */
		public function setConfigResult(array $result) {
		...
		public function getEditFormDataOfEntity(iUmiCollectionItem $entity) {
			...
			$fieldsConfig = $this->getEntityFieldsConfig(
				get_class($entity), $entity->getMap()
			);
			...
			$formData = $this->getEntityFormData(
				$fieldsConfig, $deletingEnabled, $actionMethod, $entity
			);
		...
		public function getCreateFormDataOfEntity(iUmiCollection $collection) {
			/** @var iUmiCollection|iUmiConstantMapInjector $collection */
			$fieldsConfig = $this->getEntityFieldsConfig(
				$collection->getCollectionItemClass(), $collection->getMap()
			);
			...
			$formData = $this->getEntityFormData(
				$fieldsConfig, $deletingEnabled, $actionMethod
			);
		 ...
		 * @throws RequiredPropertyHasNoValueException
		 */
		public function getEntityFormData(array $fieldsConfig, $deletingEnabled, $actionMethod, iUmiCollectionItem $entity = null) {
		...
		public function createEntityFromFormData(iUmiCollection $collection, array $createFormData) {
			...
			$createFormData = $this->callbackBeforeCreateEntityFromFormData(
				$createFormData[$entityId], $collection
			);
		 ...
		 * @return array
		 */
		public function getDomainAndLanguageQueryParams(iUmiConstantMap $constantsMap, $domainId = null, $languageId = null) {
			if ($domainId === null) {
		...
		public function getFilterValues() {
			$filtersKey = 'fields_filter';
			return (isset($_REQUEST[$filtersKey]) && is_array($_REQUEST[$filtersKey])) ? $_REQUEST[$filtersKey] : [];
		}
		...
		public function getEntitiesFilterQueryParams(iUmiCollection $collection) {
			...
			$entityFieldsConfig = $this->getEntityFieldsConfig(
				$collection->getCollectionItemClass(), $collectionConstantMap
			);
		...
		/**
		 * Возвращает путь реестра для авторизационного токена Яндекса
		 * @return string
		 */
		private function getYandexTokenRegistryPath() {
			return sprintf('//modules/%s/yandex-token', get_class($this->module));
		}
