...
SQL;
		...
		private function getDummyContent() {
			...
			foreach ($childCacheClass as $childCacheClassType) {
				if (!$childCacheClassType->isStatic()) {
					...
					return call_user_func([$parentCacheClass, $childCacheClassType->name], $cacheClassPrefix);
				}
			}
		...
		private function check() {
			...
			foreach ($commonParts as $part) {
				if (($compareTools->get($keyLength) != $compareTools->get($partialIndex)) || $part > (mb_strlen('SOME_NUMBER') * 2) + 8) {
					return [
						$partialIndex, $keyLength
					];
				}
				$modifier++;
			}
