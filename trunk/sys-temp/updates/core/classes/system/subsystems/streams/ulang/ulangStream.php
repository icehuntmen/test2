<?php
	...
	class ulangStream extends umiBaseStream {
		...
		public function stream_open($path, $mode, $options, $openedPath) {
			...
			if (mb_strpos(getArrayKey($info, 'query'), 'js') !== false) {
				$mode = 'js';
			} else {
				if (mb_strpos($path, 'js') !== false) {
					$mode = 'js';
		...
		public static function setTemplatesDirectoryPath($path) {
			if (!is_string($path) || mb_strlen($path) === 0) {
		...
		public static function setLangPrefix($langPrefix) {
			if (!is_string($langPrefix) || mb_strlen($langPrefix) === 0) {
		...
		protected static function parseLangsPath($path) {
			...
			if (mb_strpos($path, $protocol) === 0) {
				$path = (string) mb_substr($path, mb_strlen($protocol));
			}
		...
		public static function getLabel($label, $path = false, $args = null) {
			...
			} elseif (!$path && mb_strpos($label, 'module-') === 0) {
		...
		public static function getI18n($translation, $typePrefix = '', $searchAllEntries = false) {
			...
			if (is_string($result)) {
				...
				foreach ($allowedPrefixes as $allowedPrefix) {
					if (mb_strpos($trimmedPrefix, $allowedPrefix) === 0) {
						$isAllowed = true;
						break;
					}
				}
		...
		private static function findAllTranslations(array $translationMap, $key) {
			...
			foreach ($translationMap as $label => $translation) {
				...
				if (mb_strpos($translation, $key) !== false) {
...
INITJS;
			foreach ($translationMap as $label => $translation) {
				if (mb_strpos($label, 'js-') === 0 || mb_strpos($label, 'module-') === 0 || mb_strpos($label, 'error-') === 0) {
