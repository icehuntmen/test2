<?php
	...
	abstract class umiObjectProperty extends umiEntinty implements iUmiObjectProperty {
		...
		/**
		 * todo: move it from here
		 * @var int $objectPropsCashSize размер кеша значений (количество кешируемых значенией)
		 */
		...
		protected function getTableName() {
			return umiBranch::getBranchedTableByObjectId($this->object_id);
		}
		...
		protected function getPropData() {
			...
			if (umiCount($cache) >= $this->objectPropsCashSize) {
			...
			if (isset($cache[$objectId])) {
				...
				$sql = <<<SQL
SELECT field_id, int_val, varchar_val, text_val, rel_val, tree_val, float_val 
FROM {$tableName} 
...
SQL;
		 ...
		 * @return bool
		 */
		public static function loadPropsData(array $objectIds, $hierarchyTypeIds = false) {
			if (umiCount($objectIds) == 0) {
				return false;
			}
			...
			if (umiCount($objectsIdsToLoad) == 0) {
				return false;
			}
...
FROM `$tableName` 
WHERE obj_id IN ($objectsIdsToLoad)
SQL;
			$result = $connection->queryResult($sql);
			$result->setFetchType(IQueryResult::FETCH_ASSOC);
...
SQL;
		...
		public static function objectsByValue(
			...
			if (!($arr_value === null || is_array($arr_value) || (int) $arr_value === -1 || (string) $arr_value === 'all' || (string) $arr_value == 'Все')) {
				$arr_value = [$arr_value];
			}
			...
			if (!((int) $arr_value === -1 || (string) $arr_value === 'all' || (string) $arr_value === 'Âñå')) {
				$s_values_filter = " AND o.{$s_col_name} " . ($arr_value === null ? 'IS NULL' : "IN ('" . implode("', '", $arr_value) . "')");
			}
			...
			if ($connection->errorOccurred()) {
				throw new coreException('Error executing db query (errno ' . $connection->errorNumber() . ', error ' . $connection->errorDescription($s_query) . ')');
			}
