...
SQL;
		...
		/**
		 * Загружает информацию о шаблоне из БД
...
SQL;
			...
			if (!empty($this->filename)) {
				...
				if (!mb_strlen($this->type)) {
					if (mb_strtolower($templateExt) == 'xsl') {
						$this->type = 'xslt';
					} elseif (mb_strtolower($templateExt) == 'tpl') {
						$this->type = 'tpls';
					} elseif (mb_strtolower($templateExt) == 'php') {
						$this->type = 'php';
					} else {
						$this->type = mb_strlen($templateExt) ? $templateExt : 'xslt';
					}
				}
