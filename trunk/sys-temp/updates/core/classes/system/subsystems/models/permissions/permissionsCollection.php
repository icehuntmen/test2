<?php
	...
	class permissionsCollection extends singleton implements iSingleton, iPermissionsCollection {
		...
		public function isAllowedMethod($ownerId, $module, $method, $ignoreSelf = false) {
			...
			if ($module == 'content' && !mb_strlen($method)) {
				return 1;
			}
			...
			if (!array_key_exists($cacheKey, $cache)) {
				$sql = "SELECT cp.method, MAX(cp.allow) FROM cms_permissions cp WHERE module = '{$module}' AND {$sqlWhere} GROUP BY module, method";
				$result = $connection->queryResult($sql);
				$result->setFetchType(IQueryResult::FETCH_ROW);
			...
			try {
				...
				foreach ($objectIds as $id) {
					...
					if ($this->isAllowedMethod($id, $module, $method)) {
						if (in_array($id, $ownerIds) || $id == $svGroupId || $this->isAllowedMethod($id, $module, $method . '.edit')) {
							$level =
...
SQL;
		...
		protected function cleanupElementPermissions($relId) {
			...
			$connection->query("DELETE FROM cms3_permissions WHERE owner_id != '{$guestId}' AND level <= {$maxLevel} AND rel_id = {$relId}");
		}
...
SQL;
		...
		public function setAllowedDomain($ownerId, $domainId, $allow = 1) {
			...
			$connection = ConnectionPool::getInstance()->getConnection();
			$sql = "DELETE FROM cms_permissions WHERE module = 'domain' AND method = '{$domainId}' AND owner_id = '{$ownerId}'";
			$connection->query($sql);

			$sql = "INSERT INTO cms_permissions (module, method, owner_id, allow) VALUES('domain', '{$domainId}', '{$ownerId}', '{$allow}')";
			$connection->query($sql);
