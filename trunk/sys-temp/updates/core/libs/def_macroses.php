<?php
	...
	function getTitleWithPrefix($title) {
		...
		if (mb_strpos($titlePrefix, '%title_string%') !== false) {
			return (string) str_replace('%title_string%', $title, $titlePrefix);
		}

		return mb_strlen($titlePrefix) ? $titlePrefix . ' ' . $title : $title;
	}
	...
	function macros_sitename() {
		...
		$siteName = $regedit->get("//settings/site_name/{$domainId}/{$langId}") ?: $regedit->get('//settings/site_name');
		return $siteName;
	}
	...
	function macros_systemBuild() {
		return Service::Registry()->get('//modules/autoupdate/system_build');
	}
