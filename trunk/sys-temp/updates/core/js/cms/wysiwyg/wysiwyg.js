...
uAdmin('.wysiwyg', function (extend) {
	...
	WYSIWYG.prototype.tinymce47.settings = {
		...
		'formatselect fontselect fontsizeselect bold italic strikethrough alignleft aligncenter alignright ' +
	...
	WYSIWYG.prototype.umielfinderFileManager = function(fieldName, url, type, win, lang, folderHash, fileHash) {
		...
		var footerHtml = '<div id="watermark_wrapper"><label for="add_watermark">' + getLabel('js-water-mark') + '</label><input type="checkbox" name="add_watermark" id="add_watermark"></div>';
...
function defineDeprecatedWysiwygFunctions(WYSIWYG) {
	...
	// noinspection JSUnusedGlobalSymbols (вызывается динамически через `eval`)
	/**
	 * @deprecated
