<?php
	...
	class FAQMacros {
		...
		public function post_question() {
			...
			$from = $umiRegistry->get('//settings/fio_from');
			$fromEmail = $umiRegistry->get('//settings/email_from');
			$adminEmail = $umiRegistry->get('//settings/admin_email');
			$domain = Service::DomainDetector()->detect();

			$variablesAdmin = [
				'domain' => $domain->getCurrentHostName(),
				'question' => $question,
				'question_link' => $domain->getCurrentUrl() . $this->module->pre_lang . '/admin/faq/edit/' . $elementId . '/'
			];
			...
			if ($this->module->isUsingUmiNotifications()) {
				...
				if ($notification instanceof MailNotification) {
					...
					if ($subjectTemplateAdmin instanceof MailTemplate) {
						$subjectAdmin = $subjectTemplateAdmin->getProcessedContent($variablesAdmin);
					}

					if ($contentTemplateAdmin instanceof MailTemplate) {
						$contentAdmin = $contentTemplateAdmin->getProcessedContent($variablesAdmin);
					}
				}
			} else {
				try {
			...
			if (!$umiRegistry->get('//modules/faq/disable_new_question_notification')) {
				$variablesUser = [
					'domain' => $domain,
					'question' => $question,
					'ticket' => $elementId
				...
				if ($this->module->isUsingUmiNotifications()) {
					...
					if ($notification instanceof MailNotification) {
						...
						if ($subjectTemplateUser instanceof MailTemplate) {
							$subjectUser = $subjectTemplateUser->getProcessedContent($variablesUser);
						}

						if ($contentTemplateUser instanceof MailTemplate) {
							$contentUser = $contentTemplateUser->getProcessedContent($variablesUser);
						}
					}
				} else {
					try {
