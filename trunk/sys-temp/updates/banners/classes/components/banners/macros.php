<?php
	...
	class BannersMacros {
		...
		public function insert($placeName = '', $marcosId = null, $showAll = false, $currentPageId = false) {
			...
			if (($daysBeforeNotification || $clicksBeforeNotification) && $umiRegistry->get('//modules/banners/last-check-date') < (time() - 3600 * 24)) {
...
END;
					...
					case 'image': {
						...
						if (mb_strlen($sUrl)) {
							$sResult = '<a href="' . $this->module->pre_lang . '/banners/go_to/' . $iObjId . '/" ' . ($bOpenInNewWindow ? 'target="_blank"' : '') . '>' . $sBannerImg . '</a>';
						}
		...
		protected function getBannerData($banner) {
			...
			if ($days && $showTillDate instanceof umiDate) {
				$tillDate = ' - ' . getLabel('date-of-expiry-banner', 'banners') . ' ' . $showTillDate->getFormattedDate() . '.';
			} elseif ($views) {
		...
		protected function sendNotificationEmail(array $bannersData) {
			...
			if ($this->module->isUsingUmiNotifications()) {
				...
				if ($notification instanceof MailNotification) {
					...
					if ($subjectTemplate instanceof MailTemplate) {
						$subject = $subjectTemplate->getProcessedContent();
			...
			$mail = new umiMail();
			$mail->setFrom($umiRegistry->get('//settings/email_from'));
			$mail->addRecipient($umiRegistry->get('//settings/admin_email'));
			...
			$umiRegistry->set('//modules/banners/last-check-date', time());
		}
		...
		public function getRefererTags($referer) {
			...
			foreach ($searchEngines as $searchEngine) {
				if (mb_strpos($referer, $searchEngine['pattern']) !== false) {
