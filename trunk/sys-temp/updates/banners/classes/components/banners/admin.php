	...
	class BannersAdmin {
		...
		public function add() {
			...
			if ($mode == 'do') {
				...
				if (isset($_REQUEST['data']['new']['show_till_date']) && !mb_strlen($_REQUEST['data']['new']['show_till_date'])) {
		...
		public function config() {
			...
			$params['config']['int:days-before-notification'] = (int) $umiRegistry->get('//modules/banners/days-before-notification');
			$params['config']['int:clicks-before-notification'] = (int) $umiRegistry->get('//modules/banners/clicks-before-notification');
			$params['config']['boolean:not-use-referer-tags'] = $umiRegistry->get('//modules/banners/not-use-referer-tags');

			if ($umiNotificationInstalled) {
				$params['config']['boolean:use-umiNotifications'] = (bool) $umiRegistry->get('//modules/banners/use-umiNotifications');
			}
