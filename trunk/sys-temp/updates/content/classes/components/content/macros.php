				...
				foreach ($site_tree as $elementId => $childs) {
					...
					if ($element) {
						...
						if (($max_depth > 0) && $element->show_submenu) {
							$item_arr['nodes:items'] = $item_arr['void:sub_items'] = (umiCount($childs) && is_array($childs)) ? $this->gen_sitemap($template, $childs, $max_depth - 1) : '';
						} else {
							$item_arr['sub_items'] = '';
						}
