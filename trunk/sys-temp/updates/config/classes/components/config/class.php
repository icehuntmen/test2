	...
	class config extends def_module {
		...
		public function deleteExtension($name = null) {
			...
			if ($registry->contains($name)) {
				$dumpPath = CURRENT_WORKING_DIR . "/sys-temp/updates/$name/$name.xml";
		...
		/**
		 * Возвращает отношение с коэффициентом между количеством
		 * иерархических типов, для которых есть отдельные таблицы
		 * для хранения контента объектов, и общим количеством
		 * иерархических типов.
		 * @return float|int
		 */
		...
		/**
		 * Переносит контент объектов заданного иерархического типа
		 * в отдельную таблицу
		 * @param int $hierarchyTypeId идентификатор иерархического типа
		 */
		public function branchTable($hierarchyTypeId = null) {
		...
		/**
		 * Переносит контент объектов заданного иерахического типа
		 * в общую таблицу
		 * @param int $hierarchyTypeId идентификатор иерархического типа
		 */
		public function mergeTable($hierarchyTypeId = null) {
			if ($hierarchyTypeId === null) {
				$hierarchyTypeId = getRequest('param0');
			}
		...
		/**
		 * Создает сообщение и отправляет уведомление
		 * супервайзерам.
