<?php
	...
	class customer extends umiObjectProxy {
		...
		public static function getTransientCustomerId() {
			return (int) Service::Session()->get('customer-id');
		}
		...
		public function tryMerge() {
			if (!Service::Auth()->isAuthorized() || !self::getTransientCustomerId()) {
				return;
			}
		...
		public function merge(iUmiObject $customer) {
			...
			$query->where('domain_id')->equals(Service::DomainDetector()->detectId());
			...
			if (!(defined('UMICMS_CLI_MODE') && UMICMS_CLI_MODE)) {
				Service::Session()->del('customer-id');
			}
		...
		public function getCurrencyId() {
			$id = $this->isUser() ? (int) $this->getValue('preffered_currency') : (int) Service::CookieJar()
			...
			$guestId = Service::SystemUsersPermissions()
				->getGuestUserId();
			$guest = umiObjectsCollection::getInstance()
				->getObject($guestId);

			$id = (int) $guest->getValue('preffered_currency');
		...
		/**
		 * Устанавливает вабранную валюту пользователя
		 * @param int|null $id идентфикикатор валюты или null
		 * @return $this
		 */
		public function setCurrencyId($id = null) {
			if ($this->getCurrencyId() === $id) {
				return $this;
			}

			if ($this->isUser()) {
				$this->setValue('preffered_currency', $id);
				return $this;
			}
			...
			if ($id === null) {
				$cookieJar->remove('customer_currency');
			} else {
				$cookieJar->set('customer_currency', $id, time() + self::getExpirationTime());
			}
		...
		protected static function getCustomerObject($customerId = false) {
			...
			if (!(defined('UMICMS_CLI_MODE') && UMICMS_CLI_MODE) && !$customerIsUser) {
				Service::Session()->set('customer-id', $customerId);
			}
