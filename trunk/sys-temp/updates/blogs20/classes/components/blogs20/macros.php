	...
	class BlogsMacros {
		...
		public function postsList($blogId = false, $template = 'default', $limit = false) {
			...
			foreach ($postList as $post) {
				...
				$lineVariables['cut'] = system_parse_short_calls($this->prepareCut($post->getValue('content'), $postLink, $template), $postId);
		...
		public function viewBlogAuthors($blogId = false, $template = 'default') {
			...
			foreach ($allUsers as $user) {
				...
				$aLine['attribute:name'] = mb_strlen(trim($name)) ? $name : $login;
		...
		public function applyTimeRange($selector, iUmiObjectType $type = null) {
			...
			if (mb_strlen($stringFrom) && mb_strlen($stringTo)) {
			...
			} elseif (mb_strlen($stringFrom) && !mb_strlen($stringTo)) {
				$arrayFrom = explode('-', $stringFrom);
				$timeFrom = mktime(0, 0, 0, $arrayFrom[1], $arrayFrom[2], $arrayFrom[0]);

				$selector->where('publish_time')->more($timeFrom);
			} elseif (mb_strlen($stringTo)) {
				$arrayTo = explode('-', $stringTo);
				$timeTo = mktime(23, 59, 59, $arrayTo[1], $arrayTo[2], $arrayTo[0]);
		...
		public function prepareContent($content) {
			$from = ['[b]', '[/b]', '[i]', '[/i]', '[s]', '[/s]', '[u]', '[/u]', '[quote]', '[/quote]', ">\r\n", "\n", '[cut]', '[/cut]'];
			$to = ['<b>', '</b>', '<i>', '</i>', '<span style="text-decoration:line-through;">', '</span>', '<span style="text-decoration:underline;">', '</span>', '<div class="quote">', '</div>', '>', "<br />\n", '', ''];
		...
		public function postEdit($postId = false, $template = 'default') {
			...
			if (getRequest('param1') == 'do') {
				...
				if (mb_strlen($sTitle) && mb_strlen($sContent)) {
		...
		public function postAdd($blogId = false, $template = 'default') {
			...
			if (mb_strlen($sTitle) && mb_strlen($sContent) && $blogId) {
			...
			if ($blogId && !mb_strlen($sTitle) && mb_strlen($sContent)) {
		...
		public function commentAdd($postId = false, $template = 'default') {
			...
			if ($postId !== false && mb_strlen($sContent) > 0) {
				...
				if ($bNeedFinalPanic) {
					...
					$cmsController = cmsController::getInstance();
					$referrerUri = $cmsController->getCalculatedRefererUri();

					if (mb_strlen($referrerUri)) {
			...
			if (!mb_strlen($sContent) && getRequest('content') !== null) {
		...
		public function editUserBlogs($blogId = false, $template = 'default') {
			...
			if ((int) $blogId > 0 || $blogId == 'new') {
				...
				if ($aBlogInfo && isset($aBlogInfo['title']) && mb_strlen($aBlogInfo['title'])) {
