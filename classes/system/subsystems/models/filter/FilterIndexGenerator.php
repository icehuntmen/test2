	...
	class FilterIndexGenerator {
		...
		private function updatePageIndex(iUmiHierarchyElement $page) {
			...
			if (is_numeric($parentId)) {
				$level = $this->getLevel();
				$sql = "SELECT `rel_id` FROM `cms3_hierarchy_relations` WHERE `level` <= $level AND `child_id` = $pageId AND `rel_id` = $parentId;";
				$connection = ConnectionPool::getInstance()->getConnection();
				try {
		...
		private function unloadEntity($entityId, $entityType = false) {
			...
			switch ($entityType) {
				...
				default: {
					throw new publicAdminException(__METHOD__ . ': correct entity type expected, ' . $entityType . ' given');
				}
			}
		}
		 ...
		 * @param iUmiEntinty $entity объект сущности
		...
		private function getOptionedValue($value) {
			...
			foreach ($value as $option) {
				...
				$optionName = $this->getObjectName($option['rel']);
				if ($optionName !== null) {
					$fullValue .= $optionName . self::MULTIPLE_VALUE_SEPARATOR;
				}
			}
			return ($fullValue == '')? null : rtrim($fullValue, self::MULTIPLE_VALUE_SEPARATOR);
		}
			...
			switch (true) {
				case $isMultiple && is_array($value) && umiCount($value) > 0: {
					...
					foreach ($value as $objectId) {
						...
						if ($objectName !== null) {
							$fullValue .= $objectName . self::MULTIPLE_VALUE_SEPARATOR;
						}
					}
					return ($fullValue == '')? null : rtrim($fullValue, self::MULTIPLE_VALUE_SEPARATOR);
				}
		...
		private function getSymlinkValue($value) {
			...
			foreach ($value as $element) {
				$pageName = $this->getStringValue($element->getName());
				if ($pageName !== null) {
					$fullValue .= $pageName . self::MULTIPLE_VALUE_SEPARATOR;
				}
		...
		private function getTagsValue($value) {
			...
			foreach ($value as $tag) {
				$tag = $this->getStringValue($tag);
				if ($tag !== null) {
					$fullValue .= $tag . self::MULTIPLE_VALUE_SEPARATOR;
				}
			}
			return ($fullValue == '')? null : rtrim($fullValue, self::MULTIPLE_VALUE_SEPARATOR);
		}
		...
		private function getCustomColumnIndex($columnName) {
			...
			switch ($filteredField->getDataType()) {
				...
				default: {
					throw new publicAdminException(__METHOD__ . ': unsupported field type: ' . $filteredField->getDataType());
				}
			}
		}
		...
		private function createTable() {
			...
			if ($indexesCount > self::MAX_KEYS_COUNT) {
				throw new maxKeysCountExceedingException(__METHOD__ . ': too many indexed columns taken, please specify another index options');
			}
			$counter = 1;
...
SQL;
		 ...
		 * @throws publicAdminException если не удалось найти фильтруемые поля среди объектных типов текущего иерархического типа
		 * @throws publicAdminException если не удалось найти корректные фильтруемые поля среди объектных типов текущего иерархического типа
		 */
		private function loadFilteredFields() {
			...
			if ($result->length() == 0) {
				throw new publicAdminException(__METHOD__ . ': filtered fields data not found in hierarchy type with id ' . $this->getHierarchyTypeId());
			}
		...
		private function getObjectTypeIdsByHierarchy() {
			...
			if (umiCount($pages) == 0) {
				throw new noObjectsFoundForIndexingException(__METHOD__ . ' pages not found by parent: ' . $parentId . ' and level: ' . $level);
			}
...
SQL;
		...
		private function setHierarchyTypeId($hierarchyTypeId) {
			if (!is_numeric($hierarchyTypeId)) {
				throw new publicAdminException(__METHOD__ . ': correct hierarchy type id expected, ' . $hierarchyTypeId . ' given');
			}
			...
			if (!$hierarchyType instanceof iUmiHierarchyType) {
				throw new publicAdminException(__METHOD__ . ': cant get hierarchy type by id, ' . $hierarchyTypeId . ' given');
			}
		...
		private function setTableName($postfix = '') {
			...
			$this->tableName = self::TABLE_NAME_PREFIX . $this->getHierarchyTypeId() . '_' . $this->getEntitiesType() . $postfix;
