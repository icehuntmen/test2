<?php
	...
	class banners extends def_module {
		...
		public function checkTimeTargeting(iUmiObject $banner) {
			...
			if (mb_strlen($targetingByMonthDays)) {
			...
			if (mb_strlen($targetingByWeekDays)) {
		 ...
		 * @return bool|int
		 */
		public function checkTagsAndCalculateWeight(iUmiObject $banner, array $pageTags, array $userTags, array $refererTags) {
			$weight = 1;
		...
		public function getVariableNamesForMailTemplates() {
			return [
				'banners-expiration-date-subject' => [],

				'banners-expiration-date-content' => [
					'parse.banners-expiration-date-item.items' => getLabel('mail-template-variable-items', 'banners'),
				],

				'banners-expiration-date-item' => [
					'bannerName' => getLabel('mail-template-variable-bannerName', 'banners'),
					'tillDate' => getLabel('mail-template-variable-tillDate', 'banners'),
					'link' => getLabel('mail-template-variable-link', 'banners'),
				],
			];
		}
