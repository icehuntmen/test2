	...
	class WebFormsMacros {
		...
		public function add($formId = false, $who = '', $template = 'webforms') {
			...
			if (mb_strlen($who)) {
				...
				if (func_num_args() > 3) {
					...
					$paramList['res_to'] = $paramList['address_select'] = '<input type="hidden" name="system_email_to" value="' . $addressId . '" />';
				}
			} else {
		...
		private function createMessage($formId, $recipient) {
			...
			try {
				...
				$data->saveEditedObject($messageId, true);
