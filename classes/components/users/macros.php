	...
	class UsersMacros {
		...
		public function handleLoginSuccess(iUmiObject $user) {
			...
			if (Service::Request()->isAdmin()) {
				...
				system_get_skinName();
				$this->module->chooseRedirect(getRequest('from_page'));
			}
		...
		public function registrate_do() {
			...
			$withoutActivation = (bool) $umiRegistry->get('//modules/users/without_act');
			...
			$variables = [
				...
				'activate_link' => $domain->getCurrentUrl() . $module->pre_lang . '/users/activate/' . $activationCode . '/',
				'login' => $login,
				'password' => $password,
			...
			if ($subject && $content) {
				...
				$emailFrom = $umiRegistry->get('//settings/email_from');
				$fioFrom = $umiRegistry->get('//settings/fio_from');
		...
		public function forget_do($template = 'default') {
			...
			$hasLogin = mb_strlen($forgetLogin) != 0;
			$hasEmail = mb_strlen($forgetEmail) != 0;
			$userId = false;
			...
			$withoutActivation = (bool) $regedit->get('//modules/users/without_act');
			...
			if ($withoutActivation || (int) $user->getValue('is_activated')) {
				...
				$fio = $user->getValue('lname') . ' ' . $user->getValue('fname') . ' ' . $user->getValue('father_name');
				$domain = Service::DomainDetector()->detect();
				$restoreLink = $domain->getCurrentUrl() . $module->pre_lang . '/users/restore/' . $activationCode . '/';
