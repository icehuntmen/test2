...
abstract class elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if ($this->tmbPath) {
			...
			if (mt_rand(1, 200) <= $this->options['tmbCleanProb']) {
				$ls = scandir($this->tmbPath);

				for ($i=0, $s = count($ls); $i < $s; $i++) {
					$pinfo = pathinfo($ls[$i]);

					if (mb_strtolower($pinfo['extension']) == 'png') {
	...
	public function name() {
		return mb_strtolower(mb_substr(get_class($this), mb_strlen('elfinderdriver')));
	}
	...
	public function mount(array $opts) {
		...
		if (!empty($this->options['accessControl'])) {
			...
			&& method_exists($this->options['accessControl'][0], $this->options['accessControl'][1])) {
				$this->access = [$this->options['accessControl'][0], $this->options['accessControl'][1]];
			}
		}
		...
		$fileInfo = [];
		$classExists = class_exists('finfo');

		if ($classExists) {
			$fileInfo = explode(';', @finfo_file(finfo_open(FILEINFO_MIME), __FILE__));
		}

		$mimeInfo = [];
		$functionExist = function_exists('mime_content_type');

		if ($functionExist) {
			$mimeInfo = explode(';', mime_content_type(__FILE__));
		}

		switch (true) {
			case ($type == 'finfo' || $type == 'auto') && $classExists && preg_match($regexp, array_shift($fileInfo)) : {
				$type = 'finfo';
				break;
			}
			case ($type == 'mime_content_type' || $type == 'auto') && $functionExist && preg_match($regexp, array_shift($mimeInfo)) : {
		...
		if (!empty($this->options['archivers']['create']) && is_array($this->options['archivers']['create'])) {
			foreach ($this->options['archivers']['create'] as $mime => $conf) {
				if (mb_strpos($mime, 'application/') === 0
				&& !empty($conf['cmd'])
				&& isset($conf['argc'])
				&& !empty($conf['ext'])
		...
		if (!empty($this->options['archivers']['extract']) && is_array($this->options['archivers']['extract'])) {
			foreach ($this->options['archivers']['extract'] as $mime => $conf) {
				if (mb_substr($mime, 'application/') === 0
				&& !empty($cons['cmd'])
				&& isset($conf['argc'])
				&& !empty($conf['ext'])
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	public function mimeAccepted($mime, $mimes= []) {
		return $mime == 'directory' || empty($mimes) || in_array($mime, $mimes) || in_array(mb_substr($mime, 0, mb_strpos($mime, '/')), $mimes);
	}
	...
	public function copyToAllowed() {
		return (bool) $this->options['copyTo'];
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	public function scandir($hash, $mimes= []) {
	...
	public function tree($hash='', $deep=0, $exclude='') {
		$hash = $hash ?: $this->encode($this->root);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function getScandir($path, $mimes= []) {
		$files = [];
	...
	protected function doSearch($path, $q, $mimes) {
		...
		foreach($this->_scandir($path) as $p) {
			...
			if (mb_strpos($name, $q) !== false) {
				...
				if ($this->URL) {
					$stat['url'] = $this->URL.str_replace($this->separator, '/', mb_substr($p, mb_strlen($this->root)+1));
				}
	...
	protected function gettmb($path) {
		if ($this->tmbURL && $this->tmbPath) {
			...
			if (mb_strpos($path, $this->tmbPath) === 0) {
	...
	protected function canCreateTmb($path, $mime) {
		return $this->tmbPath
			...
			&& mb_strpos($path, $this->tmbPath) === false // do not create thumnbnail for thumnbnail
			&& $this->imgLib
			&& mb_strpos($mime, 'image') === 0
	...
	protected function createTmb($path) {
		...
		switch ($this->imgLib) {
			...
			case 'gd':
				...
				if (isset($img) && $img &&  ($tmp = imagecreatetruecolor($tmbSize, $tmbSize)) != false) {
