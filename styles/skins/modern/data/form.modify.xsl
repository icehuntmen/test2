...
<xsl:stylesheet
		...
		<div class="editing-functions-wrapper">
			...
			<div class="toolbar clearfix">
				...
				<script>
					...
					$.ajax({
					url:'/admin/'+curent_module+'/'+del_func_name+'.xml?childs=1&amp;element='+obj_id+'&amp;allow=true&amp;csrf='
					+ csrf,
					dataType:'xml',
					success:function (data){
	...
	<xsl:template match="page[count(properties/group) = 0]|object[count(properties/group) = 0]" mode="form-modify">
		...
		<div class="panel-settings" name="g_{@name}">
			<a data-name="{@name}" data-label="{$group-title}"></a>
			<div class="title">
	...
	<!--<xsl:template match="properties/group[not(@name = 'more_params')]" mode="form-modify">-->
	...
	<xsl:template name="changePath">
		...
		<xsl:if test="../../.././@action != 'create'">
			<div class="field changeParent col-md-6" umi:type="{$type}" id="{generate-id(//copies/copy)}">
	...
	<xsl:template name="virtuals">
		...
		<xsl:if test="../../.././@action != 'create'">
			<div class="field changeParent virtuals col-md-6" id="{generate-id()}" umi:type="{$type}">
				<div class="title-edit">
					<acronym>
						<xsl:attribute name="title">
	...
	<xsl:template match="page" mode="permissions">
		<div class="panel-settings extended_fields" name="g_permissions">
			...
			<a data-name="permissions" data-label="&permissions-panel;"></a>
	...
	<xsl:template name="edit-guide-items-link">
		...
		<div>
			<a href="{$lang-prefix}/admin/data/guide_items/{$typeId}/">
	...
	<xsl:template match="@type" mode="number">
		<xsl:if test="(. = 'int' or . = 'float' or . = 'price' or . = 'counter') and (../@name != 'domain_id')">
			<xsl:attribute name="class">
	...
	<xsl:template match="udata[@module = 'backup' and @method = 'backup_panel']">
		<div class="panel-settings extended_fields" name="g_backup">
			<a data-name="backup" data-label="&backup-changelog;"></a>
			<div class="title">
				<xsl:call-template name="group-tip">
