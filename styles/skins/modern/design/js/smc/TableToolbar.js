...
var TableToolbar = function(_oControl) {
	...
	this.TableTollbarFunctions = {
		...
		csvExportButton: {
			...
			init: function(button) {
				...
				if (Control.contentType !== 'pages' || (HandleItem && HandleItem.baseMethod != 'category')) {
					$(button.element).hide();
				}
			},
			release: function() {
	 ...
	 * @param handleItem
	 * @param {String} callbackName имя функции, которая вызывается для непосредственного импорта/экспорта
	 * @returns {*}
	 */
	var isExchangeable = function(handleItem, callbackName) {
		...
		if (!control || control.objectTypesMode || control.flatMode) {
			return false;
		}

		var selectedItems = control.selectedList;
		var selectedCount = Object.keys(selectedItems).length;
		var firstItem = selectedItems[Object.keys(selectedItems)[0]];

		if (callbackName === 'exportCallback' && !firstItem.hasChilds) {
			return false;
		}

		return (firstItem.id && selectedCount === 1 && firstItem[callbackName]);
	};
	...
	var executeExchange = function(callbackName) {
		var handleItem = HandleItem;
		var allowedCallbacks = ['importCallback', 'exportCallback'];

		if (!isExchangeable(handleItem, callbackName) || allowedCallbacks.indexOf(callbackName) === -1) {
			return;
		}
		...
		var selectedItems = control.selectedList;
		var firstItem = selectedItems[Object.keys(selectedItems)[0]];
		firstItem[callbackName](firstItem.id);
	};
	...
	var __appendButton = function(container, options) {
		...
		if (typeof(options.release) === 'function') {
			el.onclick = function() {
				...
				if (HandleItem && !$(this).hasClass('disabled') || isAllwaisActive) {
			...
			el.onclick = function() {
				...
				if ($(this).hasClass('disabled') || isAllwaisActive) {
					return false;
				}
	...
	this.show = function(_HandleItem, bForce) {
		...
		var control = HandleItem.control;
		if (control.flatMode || control.objectTypesMode) {
