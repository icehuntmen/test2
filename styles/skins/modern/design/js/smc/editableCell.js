...
var editableCell = function(tableItem, htmlTableCell, propInfo, editButton) {
	 ...
	 * @param {String} type тип поля (используется для валидации значения)
	 * @public
	 */
	this.prepareSaveData = function(type) {
		if (_$editControl) {
			_self.save(_$editControl.val(), false, type);
		}
	};
	...
	var makeEditableField = function(data) {
		...
		switch (type) {
			...
			case 'counter': {
				makeEditableStringField(value, type);
				break;
			}
			...
			case 'video_file':
			case 'file': {
				makeEditableFileField(value, type);
				break;
			}
	 ...
	 * @private
	 */
	var makeEditableStringField = function(value, type) {
	 ...
	 * @private
	 */
	var makeEditableFileField = function(value, type) {
		var $container = getContainer();
		_oldCellContent = $container.html();
		_oldValue = value;
	...
	this.save = function(newValue, force, type) {
		...
		if (!_.isArray(newValue) && !force && (_oldValue == newValue || !validateValue(newValue, type))) {
			this.restore();
			return false;
		}
