<?php
	...
	class EditInPlace {
		...
		protected static function saveFieldValue($name, $value, $target, $save = false) {
			...
			Security::getInstance()->checkCsrf();
		...
		public function isThumb($sImagePath) {
			return mb_strpos($sImagePath, '/cms/autothumbs/') !== false || mb_strpos($sImagePath, '/cms/thumbs/') !== false;
		}
		...
		public function isIeditorImage($sImagePath) {
			return mb_strpos($sImagePath, $this->getIeditorImagesPath()) !== false;
		}
		...
		public function getImageData($sImagePath = '') {
			...
			if (mb_strpos($sImagePath, CURRENT_WORKING_DIR) === false) {
		...
		public function generateOriginalImagePath($arPathInfo, $arAdditionalInfo = []) {
			...
			return $sIeditorImagesFolder . '/' . base64_encode(str_replace(CURRENT_WORKING_DIR, '', $arPathInfo['dirname']) . '/' . $arPathInfo['basename'] . $sSeparator . implode($sSeparator, $arAdditionalInfo)) . '.' . $arPathInfo['extension'];
		}
		...
		public function ieditor_crop($sImagePath){
			...
			$sNewOriginalImagePath = $this->generateOriginalImagePath(pathinfo($sImagePath), [$iSelectionWidth, $iSelectionHeight, $iSelectionLeft, $iSelectionTop]);
