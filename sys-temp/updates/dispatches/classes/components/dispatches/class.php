<?php
	...
	class dispatches extends def_module {
		...
		public function release_send_full($dispatchId) {
			...
			$mailer->setFrom($umiRegistry->get('//settings/email_from'), $umiRegistry->get('//settings/fio_from'));
			...
			foreach ($sel->result() as $recipient) {
				...
				$recipientName =
					$subscriber->getValue('lname') . ' ' .
					$subscriber->getValue('fname') . ' ' .
					$subscriber->getValue('father_name');
				...
				if ($notification instanceof MailNotification && $this->isUsingUmiNotifications()) {
					...
					if ($subjectTemplate instanceof MailTemplate) {
						$subject = $subjectTemplate->getProcessedContent(['header' => $contentVariables['header']]);
					}

					if ($contentTemplate instanceof MailTemplate) {
						$content = $contentTemplate->getProcessedContent($contentVariables);
					}
				} else {
					try {
		...
		public function getVariableNamesForMailTemplates() {
			return [
				'dispatches-release-subject' => [
					'header' => getLabel('mail-template-variable-header', 'dispatches'),
				],
				'dispatches-release-content' => [
					'header' => getLabel('mail-template-variable-header', 'dispatches'),
					'parse.dispatches-release-message.messages' => getLabel('mail-template-variable-messages', 'dispatches'),
					'unsubscribe_link' => getLabel('mail-template-variable-unsubscribe_link', 'dispatches'),
				],
				'dispatches-release-message' => [
					'header' => getLabel('mail-template-variable-header', 'dispatches'),
					'id' => getLabel('mail-template-variable-id', 'dispatches'),
					'body' => getLabel('mail-template-variable-body', 'dispatches'),
				],
				'dispatches-subscribe-subject' => [],
				'dispatches-subscribe-content' => [
					'unsubscribe_link' => getLabel('mail-template-variable-unsubscribe_link', 'dispatches')
				]
			];
		}
