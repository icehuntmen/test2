<?php
	...
	class EmarketAdmin {
		...
		public function config() {
			...
			$params['emarket-options']['boolean:delivery-with-address'] = $umiRegistry->get('//modules/emarket/delivery-with-address');
			$params['emarket-options']['boolean:purchasing-one-step'] = $umiRegistry->get('//modules/emarket/purchasing-one-step');
		...
		public function mail_config() {
			...
			if ($umiNotificationInstalled) {
				$params['status-notifications']['boolean:use-umiNotifications'] = $umiRegistry->get('//modules/emarket/use-umiNotifications');
			}
			...
			foreach ($domains as $domain) {
				...
				if (
					...
					$seo_info['string:email-' . $domain_id] = $umiRegistry->get("//modules/emarket/from-email/{$domain_id}");
					$seo_info['string:name-' . $domain_id] = $umiRegistry->get("//modules/emarket/from-name/{$domain_id}");
					$seo_info['string:manageremail-' . $domain_id] = $umiRegistry->get("//modules/emarket/manager-email/{$domain_id}");
				}
		...
		public function yandex_market_config() {
			...
			foreach ($domains as $domain) {
				...
				$domain_name = $domain->getHost();
				$config = [];
				$config["string:clientId-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/clientId");
				$config["string:password-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/password");
				$config["string:token-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/token");
				$config["string:login-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/login");
				$config["string:marketToken-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/marketToken");
				$config["string:marketCampaignId-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/marketCampaignId");
				$config["boolean:cashOnDelivery-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/cashOnDelivery");
				$config["boolean:cardOnDelivery-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/cardOnDelivery");
				$config["boolean:shopPrepaid-{$domain_id}"] = $umiRegistry->get("//modules/emarket/yandex_market/{$domain_id}/shopPrepaid");
