...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="udata" mode="order-items">
		...
		<div class="panel-settings" name="g_order_items">
			...
			<a data-name="{@name}" data-label="&label-order-items-group;"/>
			<div class="title">
				<xsl:call-template name="group-tip">
	...
	<!-- Шаблон названия наименования -->
	<xsl:template match="object" mode="order-item-name">
		<xsl:value-of select="@name"/>
	</xsl:template>

	<!-- Шаблон ссылки на товар -->
	...
	<xsl:template match="udata[@method = 'ordersList' and count(items/item)]">
		<xsl:param name="customer-id"/>
		<div class="panel-settings" name="g_orders_list">
			<summary class="group-tip">
				<xsl:text>История заказов данного покупателя.</xsl:text>
			</summary>
			<a data-name="{@name}" data-label="&label-customer-order;"/>
			<div class="title">
				<xsl:call-template name="group-tip">
