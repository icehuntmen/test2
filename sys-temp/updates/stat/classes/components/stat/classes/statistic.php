	...
	class statistic {
		...
		public function run() {
			...
			if ($this->issetSessionStatByKey('doLogin')) {
				...
				if ($this->issetSessionStatByKey('loginId')) {
					...
					$qry = "SELECT `id` FROM `cms_stat_users` WHERE `login` = '" . $connection->escape($login) . "' AND `host_id` = " . $siteId;
			...
			if (!$this->issetSessionStatByKey('id')) {
				...
				if ($cookieJar->isExists('stat_id')) {
					...
					if (isset($row['id'])) {
						...
						if (isset($row['ts']) && ($this->time - $row['ts']) / 60 <= STAT_RETURN_INTERVAL) {
							...
							$rowId = (int) $row['id'];
							$queryResult = $connection->queryResult('SELECT COUNT(*) AS `cnt` FROM `cms_stat_hits` WHERE `path_id` = ' . $rowId);
							$queryResult->setFetchType(IQueryResult::FETCH_ASSOC);
							$row = $queryResult->fetch();
				...
				if (mb_strpos($url_array['host'], 'www.') === 0) {
							...
							if (isset($arr[$row['varname']])) {
								...
								if (isset($row['id'])) {
									...
									if (isset($row['id'])) {
										...
										$qry = 'INSERT INTO `cms_stat_sources_search` (`engine_id`, `text_id`) VALUES (' . $engine_id . ', ' . $rowId . ')';
										$connection->query($qry);
						...
						if (!isset($row['group_id']) || $row['group_id'] != $this->getSessionStatByKey('site_id')) {
							...
							if (isset($row['id'])) {
								...
								if (isset($row['id'])) {
									...
									$qry = "INSERT INTO `cms_stat_sources_sites` (`uri`, `domain`) VALUES ('" . $connection->escape($uri_ref) . "', " . $rowId . ')';
									$connection->query($qry);
									$source_id = $connection->insertId();
								} else {
				...
				if (!$this->issetSessionStatByKey('path_id') || true) {
					...
					// установка path_id
					$qry = "INSERT INTO `cms_stat_paths` (`user_id`, `date`, `host_id`, `source_id`) VALUES ($userId, '$date', $siteId, $sourceId)";
					$connection->query($qry);
			...
			if (isset($row['id'])) {
				...
				$siteId = (int) $this->getSessionStatByKey('site_id');
				$qry = "INSERT INTO `cms_stat_pages` (`uri`, `host_id`, `section`) VALUES ('" . $uri . "', '" . $siteId . "', '" . $section . "')";
				$connection->query($qry);
			...
			$anotherPrevPageId = $this->issetSessionStatByKey('prev_page_id') ? ', ' . (int) $this->getSessionStatByKey('prev_page_id') : '';
			$page_id = (int) $page_id;
			$pathId = (int) $this->getSessionStatByKey('path_id');
			...
			$qry = 'INSERT INTO `cms_stat_hits` (`page_id`, `date`, `hour`, `day_of_week`, `week`, `day`, `month`, `year`, `path_id`, `number_in_path`' . $prevPageId . ') VALUES
					 (' . $page_id . ", '" . $this->getNow() . "', HOUR('" . $this->getNow() . "'), DATE_FORMAT('" . $this->getNow() . "', '%w'), WEEK('" . $this->getNow() . "'), DAY('" . $this->getNow() . "'), MONTH('" . $this->getNow() . "'), YEAR('" . $this->getNow() . "'), " . $pathId . ', ' . $numberInPath . $anotherPrevPageId . ')';
			$connection->query($qry);
		...
		public function event($name) {
			...
			if (is_array($row) && isset($row['id'])) {
				...
				$qry = "INSERT INTO `cms_stat_events` (`description`, `name`, `type`, `profit`, `host_id`) VALUES ('" . $name . "', '" . $name . "', 2, 0, " . $siteId . ')';
				$connection->query($qry);
		...
		private function addUser() {
			...
			$qry = "INSERT INTO `cms_stat_users` (`session_id`, `first_visit`, `login`, `os_id`, `browser_id`, `ip`, `location`, `js_version`, `host_id`) VALUES
					('" . $this->getSessionId() . "', '" . $this->getNow() . "', '" . $login . "', '" . (int) $this->getOsId() . "', '" . (int) $this->getBrowserId() . "', '" . $connection->escape($this->getRemoteAddr()) . "', '" . $location . "', '" . $jsVersion . "', " . $siteId . ')';
			$connection->query($qry);
		...
		private function isSearchBot() {
			...
			foreach ($this->_robots as $robot) {
				if (mb_strpos(mb_strtolower($this->_agent), mb_strtolower($robot)) !== false) {
					return true;
				}
			}
