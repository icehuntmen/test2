	...
	class SearchMacros {
		...
		public function search_do($template = 'default', $searchString = '', $types = '', $branches = '', $perPage = 0) {
			...
			foreach ($pageIds as $index => $pageId) {
				...
				$itemVariables['void:quant'] = ($index < umiCount($pageIds) - 1 ? search::parseTemplate($templateLineQuant, []) : '');
		...
		public function sphinxSearch($template = 'default', $searchString = '', $perPage = 0, $p) {
			...
			foreach ($resultMatches as $num => $element) {
				...
				$item['void:quant'] = ($num < umiCount($resultMatches) - 1 ? search::parseTemplate($templateLineQuant, []) : '');
