...
class elFinder {
	...
	protected function open($args) {
		...
		// get current working directory files list and add to $files if does not exist in it
		if (($ls = $volume->scandir($cwd['hash'], $args['mimes'])) === false) {
			return ['error' => $this->error(self::ERROR_OPEN, htmlspecialchars($cwd['name']), $volume->error())];
		}
	...
	protected function ls($args) {
		...
		if (($volume = $this->volume($target)) == false) {
			return ['error' => $this->error(self::ERROR_OPEN, '#'.htmlspecialchars($target), self::ERROR_DIR_NOT_FOUND)];
		}
		if (($list = $volume->ls($target)) === false) {
			return ['error' => $this->error(self::ERROR_OPEN, '#'.htmlspecialchars($target), $volume->error())];
		}
	...
	protected function tree($args) {
		...
		if (($volume = $this->volume($target)) == false) {
			return ['error' => $this->error(self::ERROR_OPEN, '#'.$target, self::ERROR_DIR_NOT_FOUND)];
		}
	...
	protected function tmb($args) {
		...
		foreach ($targets as $target) {
			if (($volume = $this->volume($target)) == false) {
				...
				$result['tmb_warnings'][] = $this->error(self::ERROR_OPEN, '#'.htmlspecialchars($target), self::ERROR_FILE_NOT_FOUND);
	...
	protected function file($args) {
		...
		if (($volume = $this->volume($target)) == false) {
			return ['error' => self::$errors[self::ERROR_FILE_NOT_FOUND], 'header' => $h404, 'raw' => true];
		}
		...
		if (!$file['read']) {
			return ['error' => self::$errors[self::ERROR_ACCESS_DENIED], 'header' => $h403, 'raw' => true];
		}
	...
	protected function size($args) {
		...
		foreach ($args['targets'] as $target) {
			...
			|| ($file = $volume->file($target)) == false) {
				return ['error' => $this->error(self::ERROR_OPEN, '#'.htmlspecialchars($target), self::ERROR_FILE_NOT_FOUND)];
			}

			if (!$file['read']) {
				return ['error' => $this->error(self::ERROR_OPEN, htmlspecialchars($file['name']), self::ERROR_PERM_DENIED)];
			}
	...
	protected function mkdir($args) {
		$target = $args['target'];
		$name   = $args['name'];
		$error  = [self::ERROR_MKDIR, $name];

		if (($volume = $this->volume($target)) == false
		|| ($dir = $volume->dir($target, false, true)) == false) {
			return ['error' => $this->error($error, self::ERROR_TRGDIR_NOT_FOUND, '#'.htmlspecialchars($target))];
		}

		if (!$dir['read']) {
			return ['error' => $this->error($error, self::ERROR_PERM_DENIED)];
		}
		...
		return $this->trigger('mkdir', $volume, ['added' => [$dir]]);
	}
	...
	protected function mkfile($args) {
		...
		return $this->trigger('mkfile', $volume, ['added' => [$file]]);
	}
	...
	protected function rename($args) {
		...
		||  ($rm  = $volume->file($target)) == false) {
			return ['error' => $this->error(self::ERROR_RENAME, '#'.htmlspecialchars($target), self::ERROR_FILE_NOT_FOUND)];
		}
		...
		if (($file = $volume->rename($target, $name)) == false) {
			return ['error' => $this->error(self::ERROR_RENAME, htmlspecialchars($rm['name']), $volume->error())];
		}
		...
		return $this->merge([], $this->trigger('rename', $volume, ['removed' => [$target], 'added' => [$file], 'removedDetails' => [$rm]]));
	}
	...
	protected function duplicate($args) {
		...
		foreach ($targets as $target) {
			...
			||  ($src = $volume->file($target)) == false) {
				$result['warning'] = $this->error(self::ERROR_COPY, '#'.htmlspecialchars($target), self::ERROR_FILE_NOT_FOUND);
				break;
			}

			if (($file = $volume->duplicate($target)) == false) {
				$result['warning'] = $this->error(self::ERROR_COPY, htmlspecialchars($src['name']), $volume->error());
				break;
			}

			$result = $this->merge($result, $this->trigger('duplicate', $volume, ['added' => [$file], 'src' => $src]));
		}
	...
	protected function rm($args) {
		...
		foreach ($targets as $target) {
			if (($volume = $this->volume($target)) == false
			|| ($file = $volume->file($target)) == false) {
				$result['warning'] = $this->error(self::ERROR_RM, '#'.htmlspecialchars($target), self::ERROR_FILE_NOT_FOUND);
				break;
			}

			if (!$volume->rm($target)) {
				$result['warning'] = $this->error(self::ERROR_RM, htmlspecialchars($file['name']), $volume->error());
				break;
			}

			$result = $this->merge($result, $this->trigger('rm', $volume, ['removed' => [$target], 'removedDetails' => [$file]]));
		}
	...
	protected function paste($args) {
		...
		|| ($dstDir = $dstVolume->dir($dst)) == false) {
			return ['error' => $this->error($error, '#'.$targets[0], self::ERROR_TRGDIR_NOT_FOUND, '#'.htmlspecialchars($dst))];
		}

		if (!$dstDir['write']) {
			return ['error' => $this->error($error, '#'.$targets[0], self::ERROR_PERM_DENIED)];
		}
	...
	protected function get($args) {
		...
		return ($content = $volume->getContents($target)) === false
			? ['error' => $this->error(self::ERROR_OPEN, '#'.htmlspecialchars($target), $volume->error())]
			: ['content' => $content];
	}
	...
	protected function put($args) {
		...
		return $this->trigger('put', $volume, ['changed' => [$file]]);
	}
	...
	protected function extract($args) {
		...
		if (($added = $volume->extract($target)) === false) {
			return ['error' => $this->error(self::ERROR_EXTRACT, htmlspecialchars($file['name']), $volume->error())];
		}

		return $this->trigger('extract', $volume, ['added' => $added]);
	}
	...
	protected function archive($args) {
		...
		return $this->trigger('archive', $volume, ['added' => [$archive]], $mimes);
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function trigger($cmd, $volumes, $result, $mimes= []) {
		$data = [
			'cmd'     => $cmd,
			'volumes' => is_array($volumes) ? $volumes : [$volumes],
			'result'  => $result
		];
		$volumes = is_array($volumes) ? $volumes : [$volumes];
		...
		if (!empty($result['added']) && is_array($result['added'])) {
			...
			$result['added'] = array_merge([], $result['added']);
		...
		if (!empty($result['changed']) && is_array($result['changed'])) {
			...
			$result['changed'] = array_merge([], $result['changed']);
