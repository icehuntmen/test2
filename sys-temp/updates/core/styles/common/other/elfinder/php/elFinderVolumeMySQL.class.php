...
class elFinderVolumeMySQL extends elFinderVolumeDriver {
	...
	protected function _fopen($path, $mode='rb') {
		if ($this->tmpPath && ($file = $this->stat($path)) && $file['read']) {
			...
			if ($fp) {
				$res = $this->db->query('SELECT content FROM '.$this->tbf.' WHERE id="'.$path.'"');
	...
	protected function _copy($source, $targetDir, $name='') {
		...
		$sql = 'INSERT INTO '.$this->tbf.' (parent_id, name, content, size, mtime, mime, width, height)  '
				.'SELECT "'. (int) $targetDir .'", "'.$this->db->real_escape_string($name).'", content, size, "'.time().'", mime, width, height FROM '.$this->tbf.' WHERE id="'. (int) $source .'"';
	...
	protected function _move($source, $targetDir, $name='') {
		...
		$this->clearstat();
		$sql = 'UPDATE '.$this->tbf.' SET parent_id="'.$this->_dirname($source).'", name="'.$this->db->real_escape_string($name).'", mtime="'.time().'" WHERE id="'. (int) $source .'"';
	...
	protected function _unlink($path) {
		$this->clearstat();
		$sql = 'DELETE FROM '.$this->tbf.' WHERE id="'. (int) $path .'" AND mime!="directory" LIMIT 1';
		return $this->query($sql) && $this->db->affected_rows > 0;
	}
	...
	protected function _rmdir($path) {
		$this->clearstat();
		$sql = 'SELECT COUNT(f.id) AS num FROM '.$this->tbf.' WHERE parent_id="'. (int) $path .'" GROUP BY f.parent_id';
		$res = $this->query($sql);
	...
	protected function _getContents($path) {
		$sql = 'SELECT content FROM '.$this->tbf.' WHERE id='. (int) $path;
		$res = $this->query($sql);
	...
	protected function _filePutContents($path, $content) {
		$sql = 'UPDATE '.$this->tbf.' SET content="'.$this->db->real_escape_string($content).'", size="'.mb_strlen($content).'", mtime="'.time().'" WHERE id='. (int) $path;
		if ($this->query($sql)) {
			$this->clearstat();
			return true;
		}
