...
/**
	...
	this.canMultiSelect = this.options.canMultiSelect ? this.options.canMultiSelect : false;
	...
	if (typeof(this.options.getFileCallback) == 'function' && this.commands.getfile) {
		...
		this.shortcut({
			...
			callback    : function() { self.exec('getfile').fail( function () {
				var opts  = {
					cssClass  : 'elfinder-dialog-error',
					title     : self.i18n(self.i18n('error')),
					resizable : false,
				...
				self.dialog('<span class="elfinder-dialog-icon elfinder-dialog-icon-error"/>'+self.i18n('errMultiSelect'), opts);
			} ) }
		})
		.shortcut({
			pattern     : 'ctrl+enter',
...
elFinder.prototype.commands.getfile = function() {
	...
	this.exec = function(hashes) {
		...
		//umiru
		if (maxFileCountAllowed !== null && files.length>1 ){
			return dfrd.reject();
		}
		//
