...
var askSupport = function() {
	...
	var checkLicenseKey = function() {
		$.ajax({
			...
			success: function(doc) {
				...
				$('#form_body').html('<form id="support_request" action="" method="post">' + forms[0].firstChild.nodeValue + '</form>');
	...
	var sendSupportRequest = function() {
		$.ajax({
			...
			data: $('#support_request').serializeArray(),
