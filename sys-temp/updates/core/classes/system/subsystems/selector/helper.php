<?php
	...
	class selectorHelper {
		...
		public static function detectHierarchyFilters(selector $sel) {
			$relationsIds = (array) getRequest('rel');

			if (count($relationsIds) == 0 && $sel->__get('mode') == 'pages') {
			...
			foreach ($relationsIds as $relationId) {
				try {
					if ($relationId || $relationId === '0') {
						$sel->where('hierarchy')->page($relationId);
					}
					if ($relationId === '0') {
						$sel->option('exclude-nested', true);
					}
		...
		public static function detectWhereFilters(selector $sel) {
			...
			if (count($sel->__get('types')) === 1 && $sel->__get('types')[0]->objectTypeIds !== null && umiCount($objectTypeIds) === 0) {
