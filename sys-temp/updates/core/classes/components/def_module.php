<?php
	...
	abstract class def_module {
		...
		public function getSortedModulesList() {
			...
			$utilModules = ['umiRedirects', 'umiNotifications', 'umiSettings', 'trash', 'search'];
			...
			$auth = Service::Auth();
			$isTrashAllowed = $permissions->isAllowedMethod($auth->getUserId(), 'data', 'trash');
		...
		protected function setTitle($title = '', $mode = 0) {
			...
			if ($title) {
				if ($mode) {
					$cmsControllerInstance->currentTitle = $umiRegistry->get('//domains/' . $_REQUEST['domain'] . '/title_pref_' . $_REQUEST['lang']) . $title;
				} else {
		...
		private static function getXslMailTemplatePath($filePath) {
			...
			if (mb_strpos($xslFilePath, 'mail') !== false) {
		 ...
		 * @return string
		 */
		public static function parseTPLMacroses($content, $scopeElementId = false, $scopeObjectId = false, $parseVariables = []) {
		...
		public static function parseTemplateForMail($template, $arr = [], $parseElementPropsId = false, $parseObjectPropsId = false) {
			if (mb_strpos($template, 'file://') === 0) {
				...
				$templateFragment = (isset($templateURL['fragment']) && mb_strlen($templateURL['fragment'])) ? $templateURL['fragment'] : 'result';
...
// Используем tpl-шаблонизатор
		...
		public function formatMessage($message, $bSplitLongMode = 0) {
			...
			if (!(is_array($bb_from) && is_array($bb_to) && umiCount($bb_from) === umiCount($bb_to))) {
				try {
					...
					if (!(is_array($bb_from) && is_array($bb_to) && umiCount($bb_from) === umiCount($bb_to) && umiCount($bb_to))) {
						$bb_from = ['[b]', '[i]', '[/b]', '[/i]',
							'[quote]', '[/quote]', '[u]', '[/u]', "\r\n"
						];

						$bb_to = ['<strong>', '<em>', '</strong>', '</em>',
		...
		public function errorPanic() {
			...
			$errorUrl = $cmsController->errorUrl;

			if ($errorUrl) {
				// validate url
				$errorUrl = preg_replace("/_err=\d+/is", '', $errorUrl);
				while (mb_strpos($errorUrl, '&&') !== false || mb_strpos($errorUrl, '??') !== false || mb_strpos($errorUrl, '?&') !== false) {
					$errorUrl = str_replace('&&', '&', $errorUrl);
					$errorUrl = str_replace('??', '?', $errorUrl);
					$errorUrl = str_replace('?&', '?', $errorUrl);
				}
				if (mb_strlen($errorUrl) && (mb_substr($errorUrl, -1) === '?' || mb_substr($errorUrl, -1) === '&')) {
					$errorUrl = mb_substr($errorUrl, 0, mb_strlen($errorUrl) - 1);
				}
				// detect param concat
				$sUrlConcat = (mb_strpos($errorUrl, '?') === false ? '?' : '&');
				//
		...
		public function errorSetErrorPage($errorPage) {
			...
			while (mb_strpos($errorPage, '&&') !== false || mb_strpos($errorPage, '??') !== false || mb_strpos($errorPage, '?&') !== false) {
				$errorPage = str_replace('&&', '&', $errorPage);
				$errorPage = str_replace('??', '?', $errorPage);
				$errorPage = str_replace('?&', '?', $errorPage);
			}
			if (mb_strlen($errorPage) && (mb_substr($errorPage, -1) === '?' || mb_substr($errorPage, -1) === '&')) {
		...
		/**
		 * Список переведенных названий переменных для всех шаблонов уведомлений модуля.
		...
		public static function loadTemplatesMeta($filepath = '') {
			...
			for ($i = 1; $i < umiCount($arguments); $i++) {
				$templates[$i - 1] = $templates[$i - 1] ? ['#template' => $templates[$i - 1], '#meta' => ['name' => $arguments[$i], 'file' => $filepath]] : $templates[$i - 1];
			}
		...
		public static function checkHTTPReferer() {
			try {
				return \UmiCms\System\Protection\Security::getInstance()->checkReferrer();
			} catch (Exception $e) {
				return false;
			}
		}
